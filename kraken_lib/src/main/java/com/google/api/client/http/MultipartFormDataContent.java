...
/**
 * Serializes MIME "multipart/form-data" content as specified by <a href="http://tools.ietf.org/html/rfc2388">RFC
 * 2388: Returning Values from Forms: multipart/form-data</a>
 *
 * The implementation is a subclass of {@link MultipartContent} that sets the media type to
 * <code>"multipart/form-data"</code> and defaults to {@link #DEFAULT_BOUNDARY} as a boundary
 * string.
 *
 * The generated content output differs from the superclass one to specifically meet the
 * <code>"multipart/form-data"</code> RFC specifications, for example omitting redundant headers in
 * the content parts (except for nested "multipart" parts).
 *
 * <p> Shortcut method {@link #addPart(Part, String, String)} is provided in order to easily set
 * name and file name for the mandatory header <code>"Content-Disposition"</code>. This header can
 * be manually set in each part's headers using the following format (but in this case no
 * consistency checks are made and the request will most likely fail): </p>
 *
 * <code>Content-Disposition: form-data; name="user"</code>
 *
 * <p> Specifications on the "content-disposition" header (RFC 2183):<br> {@link
 * http://tools.ietf.org/html/rfc2183} </p>
 *
 * For a reference on how to build a multipart/form-data request see: <ul> <li>{@link
 * http://chxo.com/be2/20050724_93bf.html}</li> <li>{@link http://www.faqs.org/rfcs/rfc1867.html}</li>
 * </ul>
 *
 * @author Marco Salis
 * @since 1.0
...
public class MultipartFormDataContent extends MultipartContent {
    ...
    /**
     * Factory method to create {@link HttpMediaType} with media type <code>"multipart/form-data"</code>
     */
     ...
     *
     * This is a shortcut method to allow adding the specified name and (optional) filename are
     * added in the <code>"content-disposition"</code> headers for the content (as per RFC 2183 par.
     * 2). If the header is already specified in the added part, its value is overridden.
     *
     ...
     * @param dispositionFilename The optional filename of the part (usually for adding a {@link
     *                            FileContent} part)
     */
