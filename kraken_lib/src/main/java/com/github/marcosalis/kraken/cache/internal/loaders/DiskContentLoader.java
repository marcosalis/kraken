...
/**
 * 2-level cache (memory-disk) implementation of {@link ContentLoader} that uses a slightly complex
 * version the Memoizer pattern (see {@link CacheMemoizer}) that allows handling the different types
 * of cache accesses specified by {@link AccessPolicy}.
 *
 * In order to allow this, the 100% reliability of the Memoizer in terms of serial retrieval of a
 * single item by multiple threads making concurrent requests is sacrified in favor of a major
 * flexibility.
 *
 * In case the application {@link ConnectionMonitor} is registered and returns a network connection
 * fail, the set {@link AccessPolicy} is turned into {@link AccessPolicy#CACHE_ONLY} to allow
 * retrieving old data from the caches (if any) by ignoring expiration.
 *
 * All the actions on a cache, including {@link AccessPolicy#PRE_FETCH}, are blocking for now. TODO:
 * delegate pre-fetching to a separate executor and return to the caller immediately when
 * pre-fetching.<br>
 *
 * @param <R> Requests type (extending {@link CacheableRequest})
 * @param <D> Content data type
 * @author Marco Salis
 * @since 1.0
 */
@Beta
...
public final class DiskContentLoader<D> implements ContentLoader<CacheableRequest<D>, D> {
     ...
     *
     * @param memCache       The {@link ModelLruCache} to use
     * @param diskCache      The (optional) {@link ModelDiskCache} to use
     * @param expiration     Expiration offset time to use for {@link AccessPolicy}s that require
     *                       it
     * @param requestHandler The (optional) {@link RequestHandler} for this loader
     * @param connMonitor    The (optional) {@link ConnectionMonitor} for this loader
     */
    ...
    /**
     * @see ContentLoader#load(AccessPolicy, CacheableRequest, ContentUpdateCallback)
     */
    @Override
    ...
    /**
     * Fallback method to handle the case when a task isn't successful and needs to be removed from
     * the cache to avoid pollution. For some {@link AccessPolicy}s, it is also needed to replace
     * the failed new task with the old one (if any).
     *
     * @param key     The cache key string
     * @param oldTask The old, removed future task
     * @param newTask The new, failed future task to be removed
     * @param putOld  true to synchronously put the old model in the cache again if no other new
     *                tasks have been added, false otherwise
     */
