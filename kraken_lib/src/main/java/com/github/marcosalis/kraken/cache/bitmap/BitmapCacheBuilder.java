...
/**
 * Public builder to customize and initialize a {@link BitmapCache}. See {@link BitmapLruCache} and
 * {@link SimpleBitmapDiskCache} for the default memory and disk caches specifications.
 *
 * <p> <b>Example usage:</b>
 *
 * <p>
 ...
 * </pre></code>
 *
 * <p> The threading policies and executors are common to all bitmap caches, so that callers can use
 * separate caches with different settings and limit the thread count constant. The default
 * (recommended) threading policy, that sizes executors depending on the number of device CPU cores,
 * can be overridden by calling <code>{@link BitmapCacheBase#setThreadingPolicy(BitmapThreadingPolicy)}</code>
 *
 * <p> <b>TODO list:</b> <ul> <li>Save into caches a resampled/resized version of a Bitmap</li>
 * <li>Allow selection and use of other disk/memory cache policies (LFU?)</li> <li>Effective
 * automatic disk cache purge policy implementation</li> <li>Allow custom pre/post processing of the
 * downloaded bitmap</li> </ul>
 *
 * @author Marco Salis
 * @since 1.0
 */
...
public class BitmapCacheBuilder {
    ...
    /**
     * Sets the memory cache occupation limit in bytes. Use of {@link #maxMemoryCachePercentage(float)}
     * is recommended as it enforces a proportional limit on the application memory class.
     *
    ...
    /**
     * Sets the maximum percentage of application memory that the built cache can use.
     *
     * Defaults to {@link BitmapLruCache#DEFAULT_MAX_MEMORY_PERCENTAGE}. Percentages above 30% are
     * not recommended for a typical application.
     *
     * Calling this method automatically enables the memory cache.
     *
     * @param percentage A percentage value (0 < percentage <= 100)
     * @return This builder
     */
    @NonNull
    ...
    /**
     * Sets the <b>mandatory</b> disk cache directory name. Must be a valid Linux file name. Keep
     * this consistent at each cache instantiation or the disk cache will be lost.
     *
     * Calling this method automatically enables the disk cache.
     *
     * @param cacheDirectory The cache directory (with no trailing slashes)
     * @return This builder
     */
    @NonNull
    ...
    /**
     * Sets the disk cache items expiration time in seconds. This is just a hint: it's not
     * guaranteed that the actual implementation of the disk cache will automatically perform cache
     * eviction at all. Call {@link BitmapCache#clearDiskCache(ClearMode)} to manually purge old
     * bitmaps from disk.
     *
     ...
     * Calling this method automatically enables the disk cache.
     *
     * @param seconds The cache items expiration. Automatically set to {@link
     *                SimpleDiskCache#MIN_EXPIRE_IN_SEC} if less than that
     * @return This builder
     */
    @NonNull
     ...
     *
     * Defaults to {@link DefaultHttpRequestsManager#getRequestFactory()} if not set.
     *
     * @param factory The request factory to use
     * @return This builder
     */
    @NonNull
    ...
    /**
     * Sets an optional cache logging name. Only useful for debugging and cache statistics.
     *
     * @param cacheName
     * @return This builder
     */
     ...
     * @throws IOException              If the disk cache creation failed
     * @throws IllegalArgumentException If one of the mandatory configuration parameters were not
     *                                  set
     */
    @NonNull
