...
/**
 * Disk cache that stores a POJO/DTO object into its JSON representation in text files.
 *
...
public class ModelDiskCache<V> extends SimpleDiskCache<V> {
    ...
    /**
     * Builds a {@link ModelDiskCache} that stores POJO/DTO objects as (plain) text into the passed
     * sub-folder, using the passed {@link ObjectParser}.
     *
     * Note that the {@link CacheLocation#INTERNAL} cache is always used. If no internal caches are
     * present, it falls back to the external one.
     *
     * TODO: allow choosing whether to store in ext storage for failover?
     *
     * @param context    The {@link Context} to use
     * @param mapper     The {@link ObjectMapper} to use for model de/serialization
     * @param subFolder  The relative path to the cache folder where to store the cache (if it
     *                   doesn't exist, the folder is created)
     * @param modelClass The POJO object to store class type
     * @throws IOException
     */
     ...
     *
     * @param key The cache item key (must be not null)
     ...
     *
     * <b>Note on expiration:</b> This method uses the {@link File#lastModified()} method to check
     * if the cache item is expired. Sometimes in Android this value is rounded to the second, so
     * using very small expiration times can result in a cache miss even if the item would be
     * valid.
     *
     * @param key        The cache item key (must be not null)
     * @param expiration The validity of the item from its modification in milliseconds, or {@link
     *                   Long#MAX_VALUE} for no expiration
     * @return The cached item, null if not present, expired or an error occurred
     */
    @Nullable
     ...
     *
     * @param key   The cache item key (must be not null)
     * @param model The value to put (must be not null)
     * @return true if successful, false otherwise (IO error while saving the stream)
     */
     ...
     *
     * @param key The cache item key to remove (must be not null)
