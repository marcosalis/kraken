...
/**
 * Generic, abstract extension of {@link ContentProxyBase} that defines a common interface for
 * content proxies that handle {@link JsonModel}s and {@link CacheableRequest}.
 *
 * It provides methods to load and put models from and to the proxy through a {@link ContentLoader},
 * and perform common maintenance operations to the content proxy caches.
 *
 * @author Marco Salis
 * @since 1.0
 */
...
public abstract class ModelContentProxy<MODEL extends JsonModel> extends ContentProxyBase {
     ...
     * @param request  The {@link CacheableRequest}
     * @param callback A {@link ContentUpdateCallback} to perform custom operations when a model is
     *                 added
     * @return The model, or null of unsuccessful
     * @throws Exception if an exception occurred while getting the model
     */
    @Nullable
    ...
    /**
     * Forces a model object to be put into the cache.<br> This is usually not recommended except
     * for this class internal use, and must be ONLY used for injection testing purposes or when we
     * receive an updated model after a change request to the server.
     *
    ...
    /**
     * See {@link #putModel(JsonModel)}. When the cache key is not generated only from the model,
     * you can call this method to directly provide it to the content proxy.
     *
     * @param key   The key to use in the cache for this model
     * @param model The model to put into the cache
     */
