...
/**
 * Helper class containing general static utility methods and shortcuts to Android core
 * functionalities.
 *
 * @author Marco Salis
 * @since 1.0
...
public class DroidUtils {
    ...
    /**
     * Statically initialized constant that represents the number of total CPU cores in the device.
     * This number can be higher than the one returned by {@link Runtime.getRuntime().availableProcessors()},
     * as the latter returns the number of active processors that can be used by the current virtual
     * machine (and it's used as a fallback method when it's impossible to access the
     * <i>/sys/devices/system/cpu</i> folder, where this information is retrieved).
     *
     * See {@link http ://makingmoneywithandroid.com/forum/showthread.php?tid=298&pid=1663} for more
     ...
     * to access the filesystem at "/sys/devices/system/cpu"
     *
     * @return The number of cores, or the value of {@link Runtime.getRuntime().availableProcessors()}
    ...
    /**
     * Returns a recommended size for thread pools that execute CPU-bound operations. The current
     * implementation just returns the value of the runtime method {@code
     * Runtime.getRuntime().availableProcessors()}, avoiding to add the common <i>+ 1</i> to avoid
     * putting too much overhead on the Android Dalvik threads scheduling system.
     */
    ...
    /**
     * Returns a recommended size for thread pools that execute IO-bound operations (mostly network
     * requests, rather than disk accesses).
     *
     * You are guaranteed that this method will always return more than 1 and at least 1 more than
     * the {@link #getCpuBoundPoolSize()}.
     *
     * This trivial implementation just doubles the number returned by {@link
     * #getCpuBoundPoolSize()}.
     */
    ...
    /**
     * Returns the upper memory usage limit in bytes for the current application retrieved from
     * {@link ActivityManager#getMemoryClass()}. Using a percentage of this value is a reliable way
     * to limit memory caches.
     *
     * @param context The {@link Context} to retrieve the {@link ActivityManager}
     ...
     *
     * @param context The {@link Context} to retrieve the {@link WindowManager}
    ...
    /**
     * Retrieves the screen size category for the device's default display from the {@link
     * Configuration} values.
     *
     * @param context The {@link Context} to use to retrieve the {@link WindowManager}
     * @return The retrieved screen size configuration, can be one of {@link
     * Configuration#SCREENLAYOUT_SIZE_SMALL}, {@link Configuration#SCREENLAYOUT_SIZE_NORMAL},
     * {@link Configuration#SCREENLAYOUT_SIZE_LARGE}, {@link Configuration#SCREENLAYOUT_SIZE_XLARGE}
     * (<b>use the int 4 for this configuration, constant available only from API 11</b>)
     */
    ...
    /**
     * Generates an unique, stable UID that identifies the device where the user is currently
     * logged.
     *
     * Requires the <code>READ_PHONE_STATE</code> permission in the Manifest.
     *
     * @param context A {@link Context} to retrieve data from the {@link TelephonyManager} and
     *                {@link ContentResolver}
     * @param appId   An unique and possibly secret app ID to "secure" the generated value
    ...
    /**
     * Returns the {@link #android.os.Build.SERIAL} string.<br> Not guaranteed to be unique and not
     * null, only available from API 9.
     */
    @TargetApi(9)
     ...
     *
     * Note: if the passed {@link Context} is not an activity, the flag {@link
     * Intent#FLAG_ACTIVITY_NEW_TASK} will automatically be set to avoid an Android runtime
     * exception.
     *
     * @param context The {@link Context} to start the intent with
     * @param number  The phone number to send the SMS to
     * @param text    The preset text for the SMS
     * @return true if the SMS composer has been opened, false if telephony was not available on the
     * device or the phone number wasn't in the accepted format by {@link
     * PhoneNumberUtils#isWellFormedSmsAddress(String)}. In the latter case, the SMS composer is
     * shown anyway with no recipients.
     */
     ...
     * FIXME: SMS text is not displayed in Motorola devices
     *
     * As {@link #sendSms(Context, String, String)} but allows to use multiple recipients for the
     * SMS message.
     *
     * @param context The {@link Context} to start the intent with
     * @param numbers An array of recipients phone numbers
     * @param text    The preset text for the SMS
     * @return true if the SMS composer has been opened, false otherwise (for example, because the
     * device has no telephony)
     */
    ...
    /**
     * To be used with {@link android.content.Intent.ACTION_SENDTO} to send email, either in plain
     * text or HTML.
     *
    ...
    /**
     * Create a chooser intent to open the email composer with the specified single email recipient,
     * subject and message.
     *
     * @param context        The {@link Context} to start the chooser intent with
     * @param chooserMessage The (optional) message to display in the intent chooser
    ...
    /**
     * Create a chooser intent to open the email composer with the specified email recipients,
     * subject and message.
     *
     * Note: if the passed {@link Context} is not an activity, the flag {@link
     * Intent#FLAG_ACTIVITY_NEW_TASK} will automatically be set to avoid an Android runtime
     ...
     * @param context        The {@link Context} to start the chooser intent with
     * @param chooserMessage The (optional) message to display in the intent chooser
     * @param recipients     The email addresses array to send the message to
     * @param subject        The subject of the message
     * @param message        The email message
     */
    public static void sendEmail(@NonNull Context context, @Nullable String chooserMessage,
    ...
    /**
     * Returns an {@link Intent} with action {@link Intent#ACTION_VIEW} to open the Google Play page
     * for the passed package name.
     *
     * @param packageName A full, valid Google Play application package name
     */
    @NonNull
    ...
    /**
     * Creates an {@link Intent} to open the passed URI with the default application that handles
     * {@link Intent#ACTION_VIEW} for that content.
     *
     * @param uri The URI string (must be non null)
     ...
     * @param action  The action to check (see {@link Intent} docs
    ...
    /**
     * Returns the {@link InputStream} of the given path file using the passed context to retrieve
     * the {@link AssetManager}.
     *
     * @param context The {@link Context} to use to retrieve the assets
     * @param path    The path, relative to the assets root, of the file to retrieve
     * @return The {@link InputStream} of the requested file, or null if an error occurred
     * @throws IOException
     */
    @Nullable
