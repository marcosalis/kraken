 ...
 * {@link BitmapCacheBase} subclass can hold one or more (in order to be able to
 * fine-tune the size of each of them) memory caches and a disk cache, which are
 * managed separately.
 * 
 * <p>
 * Every {@link BitmapCacheBase} shares the same executors, one for querying the
 ...
 * to block. The default executors maximum thread pool size varies depending on
 * the number of CPU cores in the device. The {@link BitmapThreadingPolicy}
 * shared among all bitmap caches can be overridden by calling
 * {@link #setThreadingPolicy(BitmapThreadingPolicy)}. This class also contains
 * other static methods to manage the execution tasks and the executors
 * lifecycle.
 * 
 * @since 1.0
 * @author Marco Salis
...
public abstract class BitmapCacheBase extends ContentProxyBase implements BitmapCache,
	...
	/**
	 * Sets a custom common static {@link BitmapThreadingPolicy} for the bitmap
	 * caches, overriding the {@link DefaultBitmapThreadingPolicy} default
	 * implementation, which is recommended for most common uses.
	 * 
	 * Call this before initializing any bitmap cache, preferably in the
	 ...
	 * 
	 * @param key
	 *            The key associated to the submitted task
	 * @param callable
	 *            The {@link Callable} to execute
	 */
