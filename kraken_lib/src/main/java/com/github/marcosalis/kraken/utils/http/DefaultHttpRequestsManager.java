...
/**
 * Default implementation of {@link HttpRequestsManager}, a decorator around the Google library
 * {@link HttpRequestFactory} to build requests using the same transport and optimizing reusable
 * components. It just takes care of using the same {@link HttpTransport} for any connection and
 * uses an {@link HttpRequestFactory} initialised with default connection parameters, user agent and
 * timeouts.
 *
 * The global instance, suitable for most uses, can be retrieved by calling {@link #get()}, and
 * {@link #initialize(ConnectionKeepAliveStrategy)} must be called before accessing the request
 * facory.
 *
 * Please note that no limits are put on the number of concurrent connection at this abstraction
 * level. Clients must implement their own pooling mechanism to limit it or just rely on the wrapped
 * connection library pooling policies.
 *
 * To debug network connections executed from the {@link HttpTransport} library in a device or
 * emulator do the following:
 *
 * <ul> <li>set the DEBUG flag to true in the {@link DroidConfig} class</li> <li>add
 * <i>Logger.getLogger(HttpTransport.class.getName()).setLevel(Level.CONFIG );</i> before the
 * execute call in your code</li> <li>type <i>adb shell setprop log.tag.HttpTransport DEBUG</i> in a
 * terminal to enable debug logging for the transport class in the connected device or emulator</li>
 * </ul>
 *
 * @author Marco Salis
 * @since 1.0
...
public class DefaultHttpRequestsManager implements HttpRequestsManager {
    ...
    /**
     * Shortcut method to return the {@link DefaultHttpRequestsManager} global instance.
     */
    ...
    /**
     * Initializes the {@link DefaultHttpRequestsManager}. Call this preferably from the {@link
     * Application#onCreate()} method.
     *
     * @param strategy The {@link ConnectionKeepAliveStrategy} if {@link ApacheHttpTransport} is
     *                 used.
     */
    @CallSuper
    ...
    /**
     * Same as {@link #initialize(ConnectionKeepAliveStrategy)} with a {@link
    ...
    /**
     * <b>Only for testing purposes.</b><br> Inject a custom {@link HttpTransport} inside the
     * manager
     *
     * @param transport The {@link HttpTransport} to inject
     */
    @VisibleForTesting
     ...
     * {@inheritDoc}
     *
     * The HttpRequest objects created with this factory don't throw exceptions if the request is
     * not successful (response < 200 or >299), so you have to check the HTTP result code within the
     * response.
     */
    @NonNull
    @Override
