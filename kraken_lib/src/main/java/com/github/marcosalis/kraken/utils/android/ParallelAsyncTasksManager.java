...
/**
 * Small component that allows managing the lifecycle (execution and cancellation) of multiple
 * {@link ParallelAsyncTask}s at the same time.
 *
 * @author Marco Salis
 * @since 1.0
 */
...
public class ParallelAsyncTasksManager {
     ...
     *
     * @param task   The task to add to the manager and execute
     * @param params The params to pass to {@link ParallelAsyncTask#parallelExec(Object...)}
    ...
    /**
     * Adds a collection of {@link ParallelAsyncTask} to the manager without executing them.
     *
     * @param tasks A {@link Collection} of async tasks
     */
    ...
    /**
     * Cancels all the tasks whose class is the same as the passed one. The comparison is made by
     * equality with the tasks {@link ParallelAsyncTask#getClass()}.
     *
     * Note that the tasks are cancelled but not removed from the manager. {@link
     * #cancelAllTasks(boolean)} has still to be called.
     *
     * @param taskClass             The {@link Class} of the tasks to cancel
     * @param mayInterruptIfRunning See {@link AsyncTask#cancel(boolean)}
